...
public class FailureRate {
	public static void main(String[] args) {
		int[] stages = {1,1,1,2,2,1,3,6};
		int[] stages2 = {2,2,3,6};
		int[] stages3 = {1,1,1,2,2,1,3,6};
		solution(10, stages2);
	...
	public static int[] solution(int N, int[] stages) {
		...
		for(int i=0; i<stages.length; i++) {
			...
			System.out.println(count[stages[i]-1]);
		}
		int ctt = 1;
		for(int c : count) {
			System.out.println("count"+(ctt++)+" = "+c);
		}
		...
		for(int i=count.length-1; i>=0; i--) {
			...
			if(passCnt==0) {
				...
				rate[i]= (float)count[i]/passCnt;
			}
			System.out.println("rate["+(i+1)+"] =="+ rate[i] );
		}
		...
		A : for(int i=0; i<N; i++) {
			...
			for(int j=0; j<N; j++) { // 같은 실패율일때 순서 정렬해주기 위해 리스트에 넣기
				if(rate[j]==max) {
					System.out.println("rate["+(j+1)+"]==>>"+rate[j]);
					list.add(j+1);
					System.out.println("리스트 사이즈 =" + list.size());
					rate[j]= (float)-1.0;
				}
			}
			System.out.println("맥스랑 같은값 찾아서 리스트에 담기 끝");
			max=-0.1;
			list.sort(null);
			for(int l=0; l<list.size(); l++) {
				System.out.println("list.get("+l+") = "+list.get(l));
				answer[answerIndex++]= list.get(l);
			}
			list.removeAll(list);
		}
		for(int a : answer) {
